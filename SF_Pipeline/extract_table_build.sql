...
$$
    ...
    try {
        // Input parameters
        var sourceDatabase = P_SOURCE_DATABASE;
        var sourceSchema = P_SOURCE_SCHEMA;
        var tableName = P_TABLE_NAME;
        var arInboundDb = P_AR_INBOUND_DB;
        var recipientEmail = P_EMAIL_LIST;

        // Validate inputs
        if (!sourceDatabase || !sourceSchema || !tableName || !arInboundDb) {
            return "Error: All parameters (P_SOURCE_DATABASE, P_SOURCE_SCHEMA, P_TABLE_NAME, P_AR_INBOUND_DB) are required.";
        }

        results.push(`Processing table: ${sourceDatabase}.${sourceSchema}.${tableName}`);
        ...
        var sourceTable = sourceDatabase + '.' + sourceSchema + '.' + tableName;
        var targetTable = sourceDatabase + '.' + sourceSchema + '.' + extractTableName;
    ...
    function sendEmail(recipient, tableName, status, resultLog, errorLog, arInboundDb) {
        try {
            ...
            htmlBody += '      <tr><td><strong>Target Location</strong></td><td>'+ sourceDatabase + '.' + sourceSchema + '</td></tr>\n';
...
CALL DEV_RAW_INBOUND_DB.CEDAR_GATE_RESP.CREATE_EXTRACT_TABLE_SP(
    'DEV_RAW_INBOUND_DB',      -- P_SOURCE_DATABASE (structure source)
    'CEDAR_GATE_RESP',               -- P_SOURCE_SCHEMA
